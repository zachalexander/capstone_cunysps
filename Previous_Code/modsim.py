...
#### ZACH COMMENT ####
### Uses the Quantity library from Pint to calculate with units
######################
...
def magnitude(x):
    ...
    returns: number
    """
    print(x.magnitude if isinstance(x, Quantity) else x)
    return x.magnitude if isinstance(x, Quantity) else x


magnitude(8000 * UNITS.meter + 4 * UNITS.cm)


#### ZACH COMMENT ####
### Same function as above except for an array of numbers or Quantities
######################
...
#### ZACH COMMENT ####
### Self explanatory, gets the unit of a Quantity
######################
...
def get_unit(x):
    ...
    """
    print(x.units if isinstance(x, Quantity) else 1)
    return x.units if isinstance(x, Quantity) else 1


get_unit(8000 * UNITS.cm)
...
#### ZACH COMMENT ####
### Obtains first unit measurement in array
######################
...
printout = get_first_unit([8000 * UNITS.cm, 2000 * UNITS.meter])
print('printout of get_first_unit', printout) 
...
printout_remove_units = remove_units([8000 * UNITS.cm, 2000 * UNITS.meter])
print(printout_remove_units)
